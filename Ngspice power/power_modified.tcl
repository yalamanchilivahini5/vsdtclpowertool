...
file delete -- "switchingpower_netlist.cir" "leakagepower_netlist.cir" "outtxtfile.txt" "leakage.txt"
...
set avgpwr_uw [format "%.3f" [expr 1e+06 *$avgpwr]]
if {$avgpwr < 0} {
 puts "\n\033\[31mError:Couldn't find the power for given timeperiod -- no corresponding entries for power values.\033\[0m"
 } else {
puts "\n\033\[33mAverage Switching power is $avgpwr W or $avgpwr_uw uW.\033\[0m"
}
close $FileInput

file delete -- "outtxtfile.txt"

} elseif {$choice==2} {

#leakage power 
puts "\033\[33mEnter no.of time varying inputs such as pulse,sine present in circuit(if no such inputs enter 0):\033\[0m"
...
puts "Executing ngspice..."
exec -ignorestderr -- ngspice leakagepower_netlist.cir -o leakage.txt
...
set leakres [open "leakage.txt" r]
   while {[gets $leakres line] >= 0} {
            if [regexp -- "vnet" $line] { 
			    set lpwr [lindex $line 2]
				set lpwr_uw [format "%.3f" [expr 1e+6* $lpwr]]
            }
        }
close $leakres
puts "\n\033\[033mLeakage power is $lpwr W or $lpwr_uw uW.\033\[0m"
file delete -- "leakage.txt"
